...
class ContractorService:
    ...
    async def get_or_create(self, entity: BaseEntity, session:AsyncSession) -> ContractorWithIdEntity:
        try:
            if contractor_entity := await self.repository.read(data=entity.to_dict(), session=session):
                return to_contractor_entity(contractor_entity)
            return to_contractor_entity(await self.repository.create(data=entity.to_dict(), session=session))
