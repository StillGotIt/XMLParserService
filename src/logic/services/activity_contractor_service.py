...
class ActivityContractorService:
    ...
    async def get_or_create(self, entity: BaseEntity, session:AsyncSession) -> BaseEntity:
        try:
            ...
            return to_activity_contractor_entity(await self.repository.create(data=entity.to_dict(), session=session))
        except SQLAlchemyError as e:
            raise e

    async def bulk_create(self, entities: list[ContractorActivityEntity], session:AsyncSession) -> BaseEntity:
        try:
            return to_activity_contractor_entity(await self.repository.create(data=[entity.to_dict() for entity in entities], session=session))
        except SQLAlchemyError as e:
            raise e
